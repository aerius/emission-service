 ...
 * ae_clone_schema
 * ---------------
 * Deze functie cloned (kopieert) een schema naar een nieuw schema.
 * 
 * Cloned alleen de structuur, niet de data.
 *
 * @param source_schema Schema welke gecloned moet worden.
 * @param destination_schema Schema welke nieuw aangemaakt zal worden en welke de geclonede structuur zal bevatten.
 */
CREATE OR REPLACE FUNCTION setup.ae_clone_schema(source_schema text, destination_schema text)
	RETURNS void AS
$BODY$
DECLARE
	src_oid oid;
...
BEGIN
	...
	-- create schema (do not drop existing first, that should be an error situtation)
	EXECUTE 'CREATE SCHEMA ' || destination_schema;

	-- create sequences
	FOR object IN
		SELECT sequence_name::text FROM information_schema.SEQUENCES WHERE sequence_schema = source_schema
	LOOP
		EXECUTE 'CREATE SEQUENCE ' || destination_schema || '.' || object;
	END LOOP;
	...
	LOOP
		buffer := destination_schema || '.' || object;
		-- create table
		...
		FOR rec IN
			SELECT 
				...
				REPLACE(column_default::text, source_schema||'.', destination_schema||'.') AS columnn_default
		...
		LOOP
			EXECUTE 'ALTER TABLE ' || buffer || ' ALTER COLUMN ' || column_name || ' SET DEFAULT ' || columnn_default;
		END LOOP;
		-- create triggers
		FOR rec IN
			SELECT 
				trigger_name::text AS trigger_name, 
				action_timing::text AS trigger_timing, 
				string_agg(event_manipulation::text, ' OR ') AS trigger_events, 
				action_orientation::text AS trigger_orientation, 
				action_statement::text AS trigger_action
				...
				GROUP BY trigger_name, trigger_timing, trigger_orientation, trigger_action
		LOOP
			EXECUTE 'CREATE TRIGGER ' || rec.trigger_name || ' ' || rec.trigger_timing || ' ' || rec.trigger_events || ' ON ' || buffer || ' FOR EACH ' || rec.trigger_orientation || ' ' || rec.trigger_action;
		END LOOP;
	END LOOP;
	...
	LOOP
		...
		FOR rec IN
			SELECT 
				...
				REPLACE(pg_get_constraintdef(pg_constraint.oid), source_schema||'.', destination_schema||'.') AS constraint_def 
		...
		LOOP
			EXECUTE 'ALTER TABLE '|| buffer ||' ADD CONSTRAINT '|| rec.constraint_name ||' '|| rec.constraint_def;
		END LOOP;
	END LOOP;
	...
	LOOP
		buffer := destination_schema || '.' || quote_ident(rec.table_name);
		EXECUTE 'CREATE OR REPLACE VIEW ' || buffer || ' AS ' || rec.view_definition || ';' ;
	END LOOP;
	...
	LOOP
		EXECUTE replace(pg_get_functiondef(rec.oid), source_schema, destination_schema);
	END LOOP;
